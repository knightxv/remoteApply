...
import { Endpoint, EventSource, Message, MessageType, IEventListener, WireValue, WireValueType } from './protocol';
...
export async function triggerApply(obj: any, message: Message) {
  const { id, type, path } = message;
  let returnValue: WireValue | { value: unknown; [throwMarker]: 0 } | boolean;
  try {
    ...
    switch (type) {
      ...
      //     parent[path.slice(-1)[0]] = fromWireValue(value);
      //     returnValue = true;
      //   }
      //   break;
  ...
  return { ...wireValue, id } as WireValue;
}
...
function createProxy<T>(
  ...
  const proxy = new Proxy(target, {
    get(_target, prop) {
      if (prop === 'bfOnInit' || prop === 'bfAfterInit') {
        return;
      }
      // if (prop === 'then') {
      //   if (path.length === 0) {
...
function requestResponseMessage(ep: Endpoint, msg: Message): Promise<WireValue> {
  return new Promise(async (resolve, reject) => {
    const id = generateUUID();
    ep.addEventListener(function l(ev: WireValue) {
      if (ev.id !== id) {
        return;
      }
      ep.removeEventListener(l);
      resolve(ev);
    } as any);
    try {
      await ep.postMessage({ id, ...msg });
    } catch (err) {
      reject(err);
    }
  });
